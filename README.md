...
- **Plans** (`/projects/:id/plans`, `/projects/:id/plans/:planId`) -- project-level architectural plan library plus an opened-plan workspace with protected image viewing, saved per-plan calibration, reusable Length Line measurements, drawn line overlays, plan selection, and session-only zoom/pan/rotation controls ahead of the rectangle/crop slices.
