...
- **FF&E Catalog** (`/projects/:id/ffe/catalog`) -- printable one-item-per-page FF&E catalog with click-to-edit item text, main Rendering plus up to three selectable option renderings, browser print, direct PDF export, per-item PDF export with editable customer-approval/signature fields, an optional hide-cost mode that condenses quantity beneath the rendering in both browser and PDF output, and a session-only main-image top or center alignment control for export prep.
