...
- **Item Library** (`/projects/:id/proposal/table`) -- the proposal-owned project item workspace now presented in the UI as Item Library, with rows grouped by schedule/category, rendering uploads, image-only Plan cells, assigned Finish Library Materials for Swatch Cells, drawing/location fields, size entry, CBM, quantity units, integer-cent totals, image-aware Excel import with column mapping, and CSV/Excel/PDF exports with project image bands, centered 4:3 row imagery, polished revision headers, and formula-backed Excel totals.
