...
- **Plans** (`/projects/:id/plans`, `/projects/:id/plans/:planId`) -- project-level architectural plan library plus an opened-plan workspace with protected image viewing, saved per-plan calibration, drawn reference-line overlays, plan selection, and session-only zoom/pan/rotation controls ahead of the measurement/crop slices.
