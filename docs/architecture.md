...
The current implementation still has separate FF&E and Proposal route contracts, hooks, and database tables. Migration `0027_shared_generated_item_schema.sql` stages Proposal-view columns on `items`, default Furniture/Unassigned groupings, and a legacy Proposal Item mapping table so future slices can migrate behavior incrementally. Migration `0028_generated_item_revision_links.sql` adds canonical Generated Item references to Proposal revision history while preserving legacy Proposal Item ids for route compatibility. Migration `0029_items_ffe_visibility.sql` adds explicit FF&E visibility so non-Furniture Proposal items can stay linked to Generated Items without appearing in FF&E until Add to FF&E is chosen. Migration `0030_rooms_ffe_visibility.sql` extends that visibility model to Locations. Migration `0031_proposal_item_name.sql` persists Proposal Name on the compatibility Proposal Item row so generated-item mirroring does not infer names from Product Description. Removing an item or Location from FF&E clears FF&E visibility only; it does not delete Generated Items, Proposal rows, images, revisions, or room records. The API bridge in `api/src/lib/generatedItems.ts` can read canonical `items` by Room or Proposal Category, creates/updates linked compatibility rows for FF&E and Proposal item writes, keeps shared fields like Location and Drawings synchronized for linked FF&E-visible items, and records FF&E edits into Proposal revision history once Proposal leaves `in_progress`. The current Proposal category endpoint remains legacy-compatible until route contracts can safely switch to canonical Generated Item ids. Until later cleanup slices run, `proposal_items` remains writable compatibility storage.

Rendering and Plan Image reads use the Generated Item link as a shared image seam: FF&E image cells still request `item` / `item_plan`, Proposal image cells still request `proposal_item` / `proposal_plan`, and the API includes images from the linked counterpart when one exists.
...
- `/projects/:id/ffe/list` shows a compatibility/supporting read-only FF&E card list grouped by Proposal Category.
- `/projects/:id/ffe/table` is a compatibility route that redirects to `/projects/:id/ffe/list`.
- `/projects/:id/ffe/catalog` shows printable FF&E catalog pages with a thumbnail-first Catalog Navigator, inline item-text editing, option renderings, and customer approval markup.
