...
- feat(plans): auto-detect PDF sheet references during plan document upload
...
- refactor(ui): reset the live app shell to a neutral/slate token palette and collapse live typography to Manrope with JetBrains Mono data accents while leaving catalog document preview styling unchanged
...
- refactor(item-library): tighten default row section widths so item titles stay inside image bounds, remove oversized dead space between sections, and show row materials in a two-column grid
- refactor(item-library): align default list sections across each schedule by measuring the widest item/specs/plan/materials/pricing block and applying shared section widths to every row
- refactor(item-library): anchor two-column swatch grids from the top-left, mute metadata labels while bolding row values, and split default list rows into lightly rounded cards with small gaps
